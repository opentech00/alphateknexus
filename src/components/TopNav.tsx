    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? 'bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg shadow-sm border-b border-slate-200/80 dark:border-slate-800'
            : 'bg-white dark:bg-slate-900 dark:border-slate-800 border-b border-slate-100'
        }`}
      >
          ...
          <div className={`flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-14' : 'h-16'}`}>
            ...
            >
              ...
              <div className="hidden sm:block text-left">
                <p className="font-bold text-slate-900 dark:text-slate-100 text-sm leading-tight">Alphatek Nexus</p>
                <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest">Client Portal</p>
              </div>
            </button>
            ...
            <nav className="hidden md:flex items-center gap-1">
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.label}
                    {active && (
                      <span className="absolute -bottom-px left-3 right-3 h-0.5 bg-blue-600 dark:bg-blue-400 rounded-full" />
                    )}
                  </button>
                );
              })}
              {effectiveAdmin && (
                <a
                  ...
                  className="flex items-center gap-2 px-3.5 py-2 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                >

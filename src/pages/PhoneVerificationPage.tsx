...
import { Loader2, AlertCircle, CheckCircle2, ArrowLeft, RefreshCw, MessageCircle } from 'lucide-react';
...
const UPSTREAM_ERROR_PATTERNS = [
  'upstream connect error', 'disconnect', 'reset', 'transport failure',
  'connection failure', 'delayed connect error', 'ECONNRESET', 'ETIMEDOUT', 'fetch failed',
];
...
async function invokeWithRetry(
  fn: 'send-whatsapp-otp' | 'verify-whatsapp-otp',
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    lastRes = res as { data: null; error: { message: string } };
...
export function PhoneVerificationPage({ phone, onBack, onVerified }: PhoneVerificationPageProps) {
  const { refreshVerification } = useAuth();
  const [digits, setDigits] = useState<string[]>(['', '', '', '', '', '']);
  const [error, setError] = useState('');
  const [info, setInfo] = useState(`A 6-digit code was sent to ${maskPhone(phone)} on WhatsApp.`);
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      const res = await invokeWithRetry('send-whatsapp-otp', undefined, 3, () => setRetrying(true));
      setRetrying(false);
      const errMsg = await extractFnError(res);
      if (errMsg) {
        setError(errMsg);
      } else {
        setInfo(`A 6-digit verification code has been sent to ${maskPhone(phone)} on WhatsApp.`);
  ...
  const submitCode = async (code: string) => {
    ...
    try {
      ...
      const res = await invokeWithRetry('verify-whatsapp-otp', { code }, 3, () => setRetrying(true));
      setRetrying(false);
      const errMsg = await extractFnError(res);
      if (errMsg) {
        setError(errMsg);
        ...
        setVerified(true);
        setInfo('Phone verified successfully! Redirecting...');
  ...
  return (
    <AuthLayout
      heroTitle="Verify your WhatsApp"
      heroDesc="Enter the 6-digit code we sent to your WhatsApp to activate your account."
    >
      ...
      <div className="mb-6 text-center">
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 bg-emerald-50">
          {verified ? (
        ...
        <p className="mt-1.5 text-slate-500 text-sm leading-relaxed">
          ...
          <span className="font-semibold text-slate-700">{maskPhone(phone)}</span> on WhatsApp.
        </p>
      </div>
      ...
      {sending && (
        <div className="flex items-center justify-center gap-2.5 px-4 py-3 mb-4 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-600">
          ...
          {retrying ? 'Connection issue — retrying...' : 'Sending WhatsApp code...'}
        </div>
      )}
      ...
      <p className="text-center text-xs text-slate-400 mt-4">
        The code expires in 10 minutes. Make sure WhatsApp can receive messages on this number.
      </p>
    </AuthLayout>
  );
}
